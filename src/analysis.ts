...
function normalizeResultFiles(files: AnalysisFiles, baseDir: string): AnalysisFiles {
  return Object.entries(files).reduce<AnalysisFiles>((obj, [path, positions]) => {
...
const moveSuggestionIndexes = <T>(
  ...
  return entries.reduce<{ [index: string]: T }>((obj, [i, s]) => {
...
function mergeLegacyResults(
  ...
  const newFiles = Object.entries(newAnalysisResults.files).reduce<AnalysisFiles>((obj, [fn, s]) => {
